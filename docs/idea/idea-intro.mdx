...
:::tip
下面所有例子均用 sibyl-go-client 编写，源码可以在 [这里](https://github.com/opensibyl/sibyl2/blob/master/cmd/sibyl/subs/server/cmd_server_scenario_test.go) 找到。
:::
...
```go
// git diff 信息可以直接执行 原生 git diff 命令得到
const diffPlain = `
...
// 也可以通过利用其他平台API（如gitlab）得到diff信息
// 转成合理的格式处理即可
...
基于上一点我们可以全局搜索函数，那必然我们可以基于此做一些更高级的封装。例如查询引用次数超过一定数量的函数？
